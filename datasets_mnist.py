...
class dataloaders_1(object):
	"""
	Load train and test data loaders for Experiment 1.
...
class dataloaders_2(object):
	"""
	Load train and test data for Experiment 2.
	"""
...
class dataloaders_2_replay(object):
	"""
	Load train and test data for Experiment 2.
	"""

	def __init__(self, general_config, data_config, dataset_tr, dataset_te, verbose, buffer_size):
	...
	def training_loaders(self, tr_dataset):
		...
		# Initialize buffer:
		buffer = Subset(all_prior_dataset_tr, random.sample(range(len(all_prior_dataset_tr)), self.buffer_size))

		# ==========
		# Then divide the rest randomly, with incorporation of the buffer
		...
		for i in range(n_rest_split):
			...
			subsets_indices = remaining_indices[start:end]
			subset_dataset = ConcatDataset([Subset(tr_dataset, subsets_indices), buffer])
			# Update buffer:
			buffer = Subset(subset_dataset, random.sample(range(len(subset_dataset)), self.buffer_size))
